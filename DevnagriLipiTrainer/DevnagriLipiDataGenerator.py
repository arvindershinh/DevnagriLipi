...
def Rotation(image, angle):
    if angle <= pi/12 and angle >= -pi/12:
...
def DataGeneratingFunction(filePath, rootFolder):
    if filePath.endswith('.png'):
      ...
      angles=np.linspace(-pi/48, pi/48, 31)
      n=angles.shape[0]
      i=0
      dx, dy = 30, 30
      for i in range(n):
        ...
        Rotated_Image1.save(os.path.join(rootFolder, fname+str(i)+str(m)+fext))
        for j in [1, -1]:
            for k in [1, -1]:
                m=m+1
    ...
    if not os.path.isdir(DevnagriImageFolder):
        ...
        for fo in DevnagriFolder:
           ...
           for fi in HindiLetterFolder:
               DataGeneratingFunction(os.path.join(HindiLetterFolderPath, fi), DevnagriImageFolder)
